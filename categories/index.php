...
try {
    $stmt = $pdo->query("SELECT c.*, COUNT(p.product_id) as product_count FROM categories c LEFT JOIN products p ON c.category_name = p.category_name GROUP BY c.category_id ORDER BY c.category_id ASC");
